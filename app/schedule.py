...
def job_done(project_id, job_id, correct_result):
    ...
    db.cur.execute(f"""SELECT volunteer, result FROM Result WHERE job_id ='{job_id}' AND project_id = '{project_id}' ;""")
    results = db.cur.fetchall()

    for user_id, result in results:
        if result == correct_result: update_trust_level(user_id, 'trust_level * 0.95')
        else: update_trust_level(user_id, 'trust_level + 0.1')
...
def majority_agrees(project_id, job_id):
    db.cur.execute(f"Select result FROM Result WHERE job_id = '{job_id}' AND project_id = '{project_id}'")
...
def receive_work(project_id, job_id, volunteer_id, result):
    ...
    random_replication = single_result_query(f"SELECT random_validation FROM Project WHERE  project_id = '{project_id}'")
    quorum_size = single_result_query(f"SELECT quorum_size FROM Jobs WHERE '{job_id}' AND project_id = '{project_id}'")
...
def fill_db():
    ...
    execute ("INSERT INTO Jobs (job_id, project_id, quorum_size, done) VALUES (1,1,3, 0);")
